({
        ...
        Objaction.setCallback( helper, function( response ) {
            ...
            if(component.isValid() && response.getState() === 'SUCCESS' ) {
                ...
                component.set('v.columns', [
                    ...
                    {label: tableHeader.Billboard__c.BB_Vendor__c.label, fieldName: 'VendorLink', sortable: true, type: 'url', typeAttributes: { label: { fieldName: 'Vendor' }, target: '_self', tooltip :{ fieldName: 'Vendor' }} },
                ...
                this.showToast({
                    "title": "Error!!",
                    "type": "error",
                    "message": 'Client Billboard has '+errors[0].message
                }); 
            }
        });
    ...
    fetchCBB : function(component,event,selectd) {
        var recId = component.get("v.recordId");
        console.log('>>>>>>>>>Vendor ID>>>>>>>>>>>>'+recId);
        var CBBRec = component.get("c.getClientBB");
        CBBRec.setParams({
        ...
        CBBRec.setCallback(this, function(response) {
            if (response.getState() === "SUCCESS") {
                var rows=response.getReturnValue();
                console.log('>>>>>>>>>Client BB Rowsssss >>>>>>>>>>>>'+JSON.stringify(rows));
                for (var i = 0; i < rows.length; i++) {
                   ...
                   if(row.Client_Name__c){
                        ...
                        if(row.BB_Vendor__c){
                            row.VendorLink = '/'+row.BB_Vendor__c; 
                            row.Vendor = row.BB_Vendor__r.Name;
                        }
                }
        ...
        $A.enqueueAction(CBBRec);
    },
    
    getTotalNumberOfRecords : function(component,event,selectd) {
        var recId = component.get("v.recordId");
        console.log('>>>>>>>>>Vendor ID>>>>>>>>>>>>'+recId);
        var TotalRecaction = component.get("c.getTotalCBBRecords");
    ...
    getMoreRec:  function(component, rows,selectd){
        return new Promise($A.getCallback(function(resolve, reject) {
            var action = component.get('c.getClientBB');
    ...
    sortData: function (cmp, fieldName, sortDirection) {
       ...
       // data.sort(this.sortBy(fieldName, reverse))
        ...
        }else if(fieldName=='VendorLink'){
            data.sort(this.sortBy('Vendor', reverse))
        }else{
            data.sort(this.sortBy(fieldName, reverse));
        }
        cmp.set("v.data", data);
